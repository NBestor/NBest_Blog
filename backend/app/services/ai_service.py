"""AI summary / polish / comment / chat generation service."""
...
NIUBAO_CHAT_PROMPT = (
    "你是牛宝，一个温暖、幽默的好朋友。"
    "请用自然、口语化的语气回复，可以适当使用 emoji，"
    "回复尽量简短精炼。像真正的朋友一样聊天。"
)


def chatWithNiubao(messages: list[dict], api_key: str, base_url: str, model: str) -> dict:
    """Chat with Niubao with multi-turn conversation history."""
    logger.info(f"Niubao chat start: messages={len(messages)}")
    start = time.time()
    try:
        client = OpenAI(api_key=api_key, base_url=base_url)
        response = client.chat.completions.create(
            model=model,
            messages=[{"role": "system", "content": NIUBAO_CHAT_PROMPT}, *messages],
            max_tokens=2000,
            temperature=0.8,
            timeout=30,
        )
        ...
        logger.info(f"Niubao chat success: elapsed={elapsed:.2f}s, replyLen={len(reply)}")
